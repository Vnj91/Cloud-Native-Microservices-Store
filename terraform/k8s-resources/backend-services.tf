  ...
  spec {
    ...
    template {
      ...
      spec {
        ...
        container {
          name  = "user-service"
          image = "${var.docker_hub_username}/user-service:latest"
...
# --- DISCOVERY SERVICE ---
resource "kubernetes_deployment_v1" "discovery_service" {
  metadata {
  ...
  spec {
    ...
    template {
      ...
      spec {
        container {
          name  = "discovery-service"
          image = "vnj91/discovery-service:latest"
...
resource "kubernetes_service_v1" "discovery_service" {
  metadata {
    name = "discovery-service"
  }
